...
import { useVolunteerStore } from "@stores";
import type { Volunteer } from "@schemas";

export default function Volunteer() {
  const store = useVolunteerStore();
  const list = useVolunteerStore(state => state.store);
  ...
  const [data, setData] = React.useState<Volunteer & { idx?: number }>(defaultData);
  ...
  return (
    <div className="space-y-4">
      <p className="text-2xl font-bold"> Volunteer </p>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1"> {mode === "add" ? "Add Volunteer" : "Edit Volunteer"} </ModalHeader>
              <ModalBody>
                <Input
                  label="Organization" value={data.organization} variant="bordered"
                  onChange={(e) => setData({
                    ...data,
